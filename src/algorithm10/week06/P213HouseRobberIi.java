...
public class P213HouseRobberIi {
    ...
    class Solution {
        public int rob(int[] nums) {
            ...
            //可以分解为2个198的问题
            //max(不偷第0个房子，不偷第nums.length - 1个房子)
            int[] noFirstNums = Arrays.copyOfRange(nums,1, nums.length);
            int[] noLastNums = Arrays.copyOfRange(nums,0, nums.length - 1);

            getValue(noFirstNums);
            getValue(noLastNums);
            return Math.max(getValue(noFirstNums) , getValue(noLastNums));
        }
